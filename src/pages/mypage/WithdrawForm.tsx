...
function describeError(e: unknown): string {
  if (!(e instanceof ApiError)) return "출금 신청 중 문제가 생겼습니다.";
  switch (e.code) {
...
export function WithdrawForm() {
  ...
  const withdraw = useMutation({
    ...
    onSuccess: (res) => {
      ...
      setResult(res);
      setAmount("");
      // 출금 신청이 지갑 잔액과 거래내역(WITHDRAW 항목)을 동시에 바꾸므로 둘 다 갱신한다
  ...
  function submit(e: React.FormEvent) {
    ...
    if (!Number.isFinite(parsed) || parsed < MIN_WITHDRAW_AMOUNT) {
      setResult(null);
      setError(
        `출금 금액은 ${formatWon(MIN_WITHDRAW_AMOUNT)} 이상이어야 합니다.`,
      );
      return;
    }
    ...
    <section className="rounded-2xl border border-line bg-surface px-5 py-4">
      ...
      {result && (
        <div className="mt-3 rounded-xl border border-line bg-paper px-3 py-2.5 text-[13px]">
          <p className="font-semibold text-up">출금 신청이 완료됐습니다.</p>
